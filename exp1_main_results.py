"""
exp1_main_results.py — Section 4.2: Main Results (Table 1)
8 methods × 3 citation datasets × 10 seeds
"""
...
def train_model(model, dataset, data, seed, epochs=300, patience=50, lr=0.005, wd=5e-4):
    set_seed(seed); model = model.to(DEVICE)
    opt = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=wd)
    best_out, best_score, wait = None, -float("inf"), 0
    is_large_val = data.val_mask.sum().item() > 100

    for _ in range(epochs):
        model.train(); opt.zero_grad(); out = model(data)
        F.nll_loss(out[data.train_mask], data.y[data.train_mask]).backward(); opt.step()
        model.eval()
        with torch.no_grad():
            out_eval = model(data)
            if is_large_val:
                score = -F.nll_loss(out_eval[data.val_mask], data.y[data.val_mask]).item()
            else:
                score = (out_eval[data.val_mask].argmax(1) == data.y[data.val_mask]).float().mean().item()
...
# ── Main ──
print("=" * 60)
print("  Exp 1 — Table 1: Main Results (10 seeds)")
print("=" * 60)

datasets = {
    "Cora": load_data("Cora"), "CiteSeer": load_data("CiteSeer"), "PubMed": load_data("PubMed"),
}
...
for ds_name, (ds, data) in datasets.items():
    ...
    print(
        f"\n  [{ds_name}] SETTA config: "
        f"alpha={cfg['alpha']}, beta={cfg['beta']}, "
        f"k={cfg['k']}, num_props={cfg['num_props']}"
    )

    for method in ["MLP", "GCN", "GraphSAGE", "GIN", "GAT", "GraphTransformer", "GradGateGNN", "SETTA"]:
        accs = []
        for seed in SEEDS:
            if method == "MLP":
