...
def summarize_records(records, num_seeds):
    ...
    summary = (
        ...
        .agg(
            mean=("acc", "mean"),
            std=("acc", lambda x: float(np.std(x, ddof=0))),
            ...
            train_time_std=("train_time", lambda x: float(np.std(x, ddof=0))),
            post_time_std=("post_time", lambda x: float(np.std(x, ddof=0))),
            runtime_std=("runtime", lambda x: float(np.std(x, ddof=0))),
            amortized_runtime_std=("amortized_runtime", lambda x: float(np.std(x, ddof=0))),
        )
    )
